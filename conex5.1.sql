...
select nom_genero "Nombre genero", nom_pelicula "Nombre Pelicula", cod_copia "Codigo", count(nro_prestamo)"Numero de prestamos"
from detalle_prestamo de inner join copia co on de.cod_copia = co.nro_copia                
                         inner join pelicula pe on co.cod_pelicula = pe.cod_pelicula
                         inner join genero g on pe.cod_genero = g.cod_genero     
group by nom_genero, nom_pelicula, cod_copia
INTERSECT    
select DISTINCT nom_genero "Nombre genero", nom_pelicula "Nombre Pelicula", cod_copia "Codigo", MAX(count(cod_copia)) OVER (PARTITION BY nom_genero) "Numero prestamos"
from detalle_prestamo de inner join copia co on de.cod_copia = co.nro_copia                
                         inner join pelicula pe on co.cod_pelicula = pe.cod_pelicula
                         inner join genero g on pe.cod_genero = g.cod_genero     
group by nom_genero, nom_pelicula, cod_copia;
...
select nom_pelicula, co.cod_pelicula, co.nro_copia, count(co.nro_copia) Cantidad
...
group by nom_pelicula, co.cod_pelicula, co.nro_copia     
having count(co.nro_copia) > 2
order by nom_pelicula

select Sec1.nom_pelicula  
from (select pe.nom_pelicula, pe.cod_pelicula,count(co.nro_copia) NoCopias
     from pelicula pe inner join copia co on co.cod_pelicula=pe.cod_pelicula
     group by pe.nom_pelicula,pe.cod_pelicula) Sec1
     inner join (     
     (select Sec2.cod_pelicula, count(Sec2.nro_copia) NoCopiasMayor2
     from (select pe.nom_pelicula, pe.cod_pelicula,co.nro_copia, count(d.cod_copia) 
           from pelicula pe inner join copia co on co.cod_pelicula=pe.cod_pelicula
           inner join detalle_prestamo d on co.nro_copia = d.cod_copia   
           group by pe.nom_pelicula,pe.cod_pelicula,co.nro_copia
           having count(d.cod_copia)>'2') Sec2
           group by Sec2.cod_pelicula) Sec3
) on Sec1.cod_pelicula=Sec3.cod_pelicula
where Sec3.NoCopiasMayor2>=Sec1.NoCopias
order by Sec1.nom_pelicula 
...
select c.nm1_cliente, lower(c.nm1_cliente)
from cliente c 
...
select c.nm1_cliente, initcap(c.nm1_cliente)
from cliente c


cantidad de caracteres

select c.nm1_cliente, length(c.nm1_cliente)
from cliente c


el substr el primer dato es de donde se va a cojer, el segundo es la pocision y el tercero es cuantos datos coje de ahi en adelante 

select c.nm1_cliente, substr(c.nm1_cliente,2, 2)
from cliente c
...
select concat (c.nm1_cliente, c.nm2_cliente)
from cliente c

select (c.nm1_cliente ||' '||nm2_cliente ||' '||ap1_cliente||' '||ap2_cliente) nombres
        from cliente c
...
select round(56.489), round (56.476,2)
from dual


tunca un numero 

select trunc(56.489),trunc(56.476,2)
from dual
...
select to_char(fechana,'day/month/year')
from empleado

select round((sysdate - to_date('29/05/1977')),0) dias,  round((sysdate - to_date('29/05/1977'))/30,0) Meses,  round((sysdate - to_date('29/05/1977'))/7,0) Semanas,  (sysdate - to_date('29/05/1977'))/365 AÑo
from dual




el crea un tabla a partir de una consulta 

create table empleasetenta as 
select *
from cliente
where substr(fec_nacimiento,7,2) = 77




inner left, inner raigh
...
declare 
    ...
    v_cod    actor.cod_actor%type;
    v_nom    actor.nom_actor%type;
    
    Begin 
    
       select cod_actor, nom_actor
       into   v_cod, v_nom
       from actor
       where cod_actor= 'A01';
       
       DBMS_OUTPUT.PUT_LINE(' El actor es : ' ||v_cod||' '||v_nom);
       
     Exception
     
        When No_Data_Found Then
           DBMS_OUTPUT.PUT_LINE(' No hay Datos en la Base de Datos');
...
begin 
  ...
  select cod_cliente, nm1_cliente, nm2_cliente, ap1_cliente, ap2_cliente, fec_nacimiento, nro_hijos, sexo
  into v_id, v_n1, v_n2, v_a1, v_a2, v_fec, v_hij, v_sex
  from cliente
  where nro_hijos= (select max(nro_hijos)
                    from cliente);
dbms_output.put_line('La persona es: ' ||v_id||' '||v_n1||' '||v_n2||' ' ||v_a1||' '||v_a2||' '||v_hij);     
...
Declare
         ...
         v_np   number(20);  
begin
      select c.cod_cliente, nm1_cliente, nm2_cliente, ap1_cliente, ap2_cliente, count(nro_prestamo)
      into v_id, v_n1, v_n2, v_a1, v_a2, v_np
      from cliente c inner join prestamo p on c.cod_cliente = p.cod_cliente
      group by c.cod_cliente, nm1_cliente, nm2_cliente, ap1_cliente, ap2_cliente
      having count(nro_prestamo) = (select max(count(nro_prestamo))
                                   from cliente c inner join prestamo p on c.cod_cliente = p.cod_cliente
                                   group by c.cod_cliente, nm1_cliente, nm2_cliente, ap1_cliente, ap2_cliente);     
                                   
     dbms_output.put_line('La persona que mas prestamos ha realizo es : ' ||v_id||' '||v_n1||' '||v_n2||' ' ||v_a1||' '||v_a2||
     ' con '||v_np ||' consultas ');                                   
end;
...
CREATE OR REPLACE PROCEDURE descuento(v_codcliente varchar2) IS
      CURSOR descuen IS 
         SELECT c.cod_cliente, nm1_cliente, ap1_cliente,p.nro_prestamo, valor_multa
         FROM cliente c INNER JOIN prestamo p ON c.cod_cliente = p.cod_cliente 
                        INNER JOIN detalle_prestamo d ON p.nro_prestamo = d.nro_prestamo 
         WHERE c.cod_cliente = v_codcliente;
...
BEGIN        
        ...
        FOR i IN descuen LOOP
           DBMS_OUTPUT.PUT_LINE('Cliente :  '||i.cod_cliente||' ' ||i.nm1_cliente||' '||i.ap1_cliente||'prestamo: '||i.nro_prestamo|| 'multa :'||i.valor_multa);
        END LOOP;
        
        
       
       
        if valor_multa < 200 then
        UPDATE detalle_prestamo
        SET valor_multa = valor_multa - (nvl(valor_multa, 0) * 0.1)
       ...
       ELSE        
        ...
        UPDATE detalle_prestamo
        SET valor_multa = valor_multa - (nvl(valor_multa, 0) * 0.15)
        WHERE nro_prestamo = i.nro_prestamo;
        end if;
...
diseñe un procedimiento almacenado que reciba por pantalla el codigo del actor 
y muestre los datos de los clientes que han alquilado peliculas de ese actor 


select * from actor
desc actor

CREATE OR REPLACE PROCEDURE verActor(v_idActor varchar2) IS
     
      CURSOR mostraActor IS 
         select c.cod_cliente, c.nm1_cliente, c.ap1_cliente, nom_actor
               from cliente c inner join prestamo p on c.cod_cliente = p.cod_cliente
               inner join detalle_prestamo d on d.nro_prestamo = p.nro_prestamo
               inner join copia co on co.nro_copia = d.cod_copia
               inner join pelicula  pe on pe.cod_pelicula = co.cod_pelicula
               inner join actor_pelicula ap on pe.cod_pelicula = ap.cod_pelicula
               inner join actor a on a.cod_actor = ap.cod_actor
               where a.cod_actor = v_idActor;
         
       
BEGIN     
         
        FOR i IN mostraActor LOOP
           DBMS_OUTPUT.PUT_LINE('Persona: '||i.cod_cliente||' ' ||i.nm1_cliente||' '||i.ap1_cliente||', Actor: '||i.nom_actor);
        END LOOP;

END;

execute verActor('A01'); 



1.	Diseñe un bloque PLSQL que reciba como parámetro el código del cliente y la cantidad de hijos y actualice la cantidad de hijos del cliente.
...
Set Verify on  
Accept p_emp PROMPT 'Codigo del  empleado: '
Accept p_hij PROMPT 'Numero de hijos: '
declare  
         v_n1   varchar2(20);
         v_n2   varchar2(20);
         v_a1   varchar2(20);
         ...
         v_id   varchar2(20);
         v_hj   number(20);
        v_idCliente varchar2(20):=&p_emp;
        v_cantHijos number(20):=&p_hij;
begin     
   
   select cod_cliente, nm1_cliente, nm2_cliente, ap1_cliente, ap2_cliente, nro_hijos
   into v_id , v_n1, v_n2, v_a1, v_a2, v_hj
   from cliente
   where cod_cliente = v_idCliente;   
   
    DBMS_OUTPUT.PUT_LINE('Antes de la actualizacion');       
    DBMS_OUTPUT.PUT_LINE('Persona : '||v_id||' ' ||v_n1||' '||v_n2||' '||v_a1||' '||v_a2||' #hijos : '||v_hj);       
        
        UPDATE cliente
        SET nro_hijos = v_cantHijos
        WHERE cod_cliente = v_idCliente;               
        
        DBMS_OUTPUT.PUT_LINE('Depsues  de la actualizacion');       
        DBMS_OUTPUT.PUT_LINE('Persona : '||v_id||' ' ||v_n1||' '||v_n2||' '||v_a1||' '||v_a2||' #hijos : '||v_cantHijos); 
